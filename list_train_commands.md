...
For ESM-enabled models, set `ESM_EMBEDDINGS_DIR` to your precomputed ESM embedding directory and pass it with `--esm-embeddings-dir`. The local example above keeps generated embeddings outside the Git repository; edit it for your system. The ESM-enabled examples below also pass `--no-prepare-missing-esm-embeddings` so missing files fail clearly instead of being regenerated by accident.
