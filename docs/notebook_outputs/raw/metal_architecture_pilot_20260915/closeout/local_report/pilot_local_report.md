...
Campaign status: **incomplete_session_stopped**. Session status: **stopped_verified**.

Allocated GPU time: **1.063324742 hours** (including setup, transfers, and time until verified shutdown).

Session stop reason: Session appears to be lost (404/401). Cleaning up.
...
## Unverified final attempt and recovery

The archived attempt ledger retains 11 saved entries unchanged. The host closeout record is reported separately from that ledger.

- attempt_012 / A1_2_four_class_lr3e-05_s42: last observed at 3 epochs; completed artifacts remain unverified. Its exact start and elapsed time are unavailable. It contributes no score to the completed-run tables.

Reconcile the preserved ledger with the host's observation of the missing attempt; retain its retry history and the closed allocation interval before continuing. A later retry must be linked to that attempt. Cross-session recovery has not been tested.

No reconstructed execution ledger was written, and no resumability claim is made.
