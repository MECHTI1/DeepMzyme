...
def run_worker(ledger_path, run_id, output_dir, device):
    ...
    for name, path in (("run_config.json", config_path), ("best_model_checkpoint.pt", checkpoint_path)):
        require(entry["expected_sha256"].get(name) == digest(path), f"Artifact hash mismatch: {name}")
    ...
    receipt = {"run_id": run_id, "task": config["task"], "validation_only": True, "reproduction_passed": True,
               ...
               "path_map": path_map, "exporter_sha256": digest(__file__)}
...
def main(argv=None):
    ...
    for run_id in chosen:
        if (output / run_id).exists():
            ...
            continue
        command = [sys.executable, str(Path(__file__).resolve()), "--reuse-ledger", str(ledger_path),
