...
**Status: Phase 0 source audit implemented; structural dataset incomplete.**
The [source audit](PMM_SOURCE_AUDIT.md) provides a runnable isolated audit
command and row-wise manifests. A later
[bounded feature check](PMM_FEATURE_MATCH_FEASIBILITY.md) uniquely matched
seven of nine reviewed rows. The subsequent
[complete feature audit](PMM_FULL_FEATURE_CROSSWALK.md) processed all 9,408
rows, but only 6,791 mapped uniquely and ED-containing types were strongly
underrepresented. Its crosswalk and exclusion ledger are audit artifacts;
the structural training dataset was not materialized. This document continues
to specify the remaining
additive implementation route; it does not authorize a test run.
...
The raw GitHub class-model files contain 7,920 train and 1,488 test rows;
`dropna()` removes none. The released script then requests an absent `source`
column, so these are **not yet certified effective training/evaluation
counts**. The paper describes additional data selection and oversampling.
PMM's classification SQL verifies code `1=Mn`, `2=Fe+Co+Ni`, `6=Cu`, and
`7=Zn`. The paper reports fivefold validation on its training data, but the
released row files and verified Figshare v1 archive do not contain serialized
fold IDs. The deposited Figure 2 notebook specifies a deterministic fivefold
algorithm on source row order, but its resulting IDs are not independently
verified published fold memberships. The archive's element and coordinate
tables still lack the direct PMM internal-ID-to-ion join; see the
[deposition crosswalk](PMM_DEPOSITION_CROSSWALK.md) and
[complete feature audit](PMM_FULL_FEATURE_CROSSWALK.md). See PMM's
...
1. The PMM GitHub commit, source hashes, numeric class-code mapping, raw and
   `dropna` counts, duplicate-ID audit, Figshare v1 archive, and available
   fold algorithm have been inspected; see the [source audit](PMM_SOURCE_AUDIT.md)
   and [deposition review](PMM_DEPOSITION_CROSSWALK.md). The remaining gate is
   a source-cohort PMM/NEIGHBORHOOD internal-ID-to-chain/coordinate export,
   including provenance for `metalid` or an explicit finding that it is only
   an opaque row key. No published row-fold IDs have been serialized in the
   inspected material. A mismatch between released code, deposited data, and
   the paper is a documented finding, not a value to guess.
2. Resolve each PMM row against a pinned coordinate version. Prefer a direct
   ...
   coordinating ligands, with source-row identity established independently.
   Record author-versus-label numbering and crystal
...
evidence in `docs/notebook_outputs/`. The historical exact-PMM benchmark
document now qualifies its earlier “identical dataset/folds” wording; a direct
PMM performance claim still requires the source-site and paired-validation
gates above.
