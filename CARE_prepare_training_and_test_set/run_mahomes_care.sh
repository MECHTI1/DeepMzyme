#!/usr/bin/env bash
# Run MAHOMES on CARE-derived reduced PDB files without touching CLEAN or PinMyMetal jobs.
set -euo pipefail

WORK_ROOT="${WORK_ROOT:-/media/Data/care_sets/task1_30}"
SPLIT="${SPLIT:-train}"
...
if [[ "$CARE_JOB_DIRS" != "0" && "$CARE_JOB_DIRS" != "1" ]]; then
    echo "[ERROR] CARE_JOB_DIRS must be 0 or 1, got: $CARE_JOB_DIRS"
    exit 1
fi
...
for part_file in "$JOB_ROOT"/batch_input_part_*; do
    ...
    if [[ "$SKIP_COMPLETED_JOBS" == "1" ]] \
        ...
        echo "[SKIP] CARE job $job_index already complete for current batch"
        skipped_jobs=$((skipped_jobs + 1))
        job_index=$((job_index + 1))
        continue
    fi
    if [[ "$CARE_JOB_DIRS" == "1" ]]; then
        rm -rf "$job_dir"
    fi
    mkdir -p "$job_dir"
    (
        exec >> "$job_dir/job.log" 2>&1
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting CARE job $job_index (PID $$)"
    ...
    pids[$job_index]=$!
    echo "[LAUNCHED] CARE job $job_index (PID ${pids[$job_index]}) -> $job_dir/job.log"
    job_index=$((job_index + 1))
done
...
echo "[DONE] CARE MAHOMES $SPLIT jobs finished at $(date '+%Y-%m-%d %H:%M:%S')"
