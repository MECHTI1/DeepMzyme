#!/usr/bin/env bash
# Shared settings for the CARE Task 1 <30% identity clusterRes30 representative preparation scripts.
set -euo pipefail
...
export CARE_TRAIN_CSV="${CARE_TRAIN_CSV:-$PROJECT_ROOT/DeepMzyme_Data/CARE_dataset/CARE_datasets/splits/task1/protein_train.csv}"
export CARE_TEST_CSV="${CARE_TEST_CSV:-$PROJECT_ROOT/DeepMzyme_Data/CARE_dataset/CARE_datasets/splits/task1/30_protein_test.csv}"

export CARE_TASK="${CARE_TASK:-task1}"
export CARE_SPLIT_NAME="${CARE_SPLIT_NAME:-30_identity_clusterRes30}"
export CARE_TRAIN_REPRESENTATIVE_COLUMN="${CARE_TRAIN_REPRESENTATIVE_COLUMN:-clusterRes30}"

export WORK_ROOT="${WORK_ROOT:-/media/Data/care_sets/task1_30_clusterRes30}"
export OUTPUT_ROOT="${OUTPUT_ROOT:-/media/Data/care_sets/task1_30_clusterRes30/exported/CARE_task1_30_clusterRes30_train_test_metallo}"
