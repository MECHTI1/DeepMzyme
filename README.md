...
`DeepMzyme_Data_v3_exact_clean_esm.tar.zst` contains the exact PinMyMetal split
plus the shared ESM, external-feature, and RING assets when present:
...
PYTHONPATH=src /home/mechti/miniconda3/envs/DeepMzyme/bin/python src/build_colab_bundle.py \
  ...
  --output-bundle DeepMzyme_Data/DeepMzyme_Colab_Bundles/DeepMzyme_Data_v3_exact_clean_esm.tar.zst
```
...
- `huggingface_link`: downloads `https://huggingface.co/datasets/GMBioinformatics/DeepMzyme/resolve/main/DeepMzyme_Data_v3_exact_clean_esm.tar.zst`, verifies SHA256, and unpacks it under `/content`.
- `upload_file`: prompts for a local `.tar.zst` upload in the Colab runtime. The current exact PinMyMetal bundle is `DeepMzyme_Data_v3_exact_clean_esm.tar.zst`.
- `drive`: uses the configured Google Drive data path after Drive is mounted.

Current `DeepMzyme_Data_v3_exact_clean_esm.tar.zst` SHA256:
